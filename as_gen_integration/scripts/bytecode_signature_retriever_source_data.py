...
def check_test(bug_id, report_dir):
    ...
    if os.path.exists(os.path.join(report_dir, 'report.exec')):
        fixed_coverage = _check_coverage(fixed_dir, bug_id, report_dir)
    elif os.path.exists(fixed_dir) and os.path.exists(buggy_dir):
        ...
        raise FileNotFoundError(f"No fixed and buggy version for {bug_id} found.")
    return {
...
def process(input):
    bugID = input['extra:project_name'][:input['extra:project_name'].rfind('_')]
    methodObj = parseMethods(f"public class A {{{input['source:source_method_code_format']}}}")[0]
    return_val = methodObj['method_return_type']
    method_sig = input['source:source_method_signature'].replace('#', '.', 1)
    ...
    method_sig = method_sig[method_sig.find('#') + 1:].replace('#', '.')
...
if __name__ == '__main__':
    def b():
        ...
        for item in a:
            print(item)
    # inputs = []
    # with open(f"{code_base}/resources/source_data.jsonl", 'r', encoding="utf-8") as f:
    #     for line in f.readlines():
    #         inputs.append(json.loads(line.strip()))
    # writer = open(f'{code_base}/outputs/input_with_bytecode_sig.jsonl', 'w', encoding='utf-8')
    # idx = 0
    # if debug: inputs = inputs[:10]
    # for input in tqdm(inputs):
    #     try:
    #         bug_id = input['extra:project_name'][:input['extra:project_name'].rfind('_')]
    #         bytecode_signature = process(input)
    #         if bytecode_signature:
    #             input['bytecode_signature'] = bytecode_signature
    #             writer.write(json.dumps(input, ensure_ascii=False) + '\n')
    #         else:
    #             logger.error(f'{idx} has no valid focal method.')
