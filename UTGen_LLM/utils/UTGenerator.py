...
class BasicGenerator(AbsGenerator):
    ...
    def generate_with_prefix(self, **data):
        ...
        return prefix + response if with_prefix else response
    ...
    def generate(self, **data):
        ...
        messages = [{
            ...
            'content': self.base_instruction(focal_method_instance=focal_method_instance,
                                             focal_class_instance=focal_class_instance,
                                             other_methods=data['other_methods'] if 'other_methods' in data else [])
        }]
        response = self.model.get_response(messages)
        return response

    def base_instruction(self, focal_method_instance: Method, focal_class_instance: Class, other_methods: list = []):
        ...
        if not other_methods:
            focal_class_other_methods = [m.short_definition for m in focal_class_instance.public_methods if
                                         m.signature != focal_method_signature]
        else:
            focal_class_other_methods = other_methods
        ...
        prompt += '\n```\n'  # Comment style
        prompt += f'// The method under test is defined in the {focal_class} class, Here are the defined fields and methods in the class:\n'
