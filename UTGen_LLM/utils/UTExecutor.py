...
def cal_pass_rate(instances):
    ...
    for instance in instances:
        ...
        if expected_value in boolean_expected_value or expected_value in nullable_expected_value:
            new_test_case = instance['test_prefix'].replace('<AssertionPlaceHolder>', generated_assertion)  if 'test_prefix' in instance else comparison_instance['test_prefix'].replace('<AssertionPlaceHolder>', generated_assertion)
        else:
            new_test_case = replace_assertion(instance['test_prefix'], generated_assertion) if 'test_prefix' in instance else replace_assertion(comparison_instance['test_prefix'], generated_assertion)
            if not new_test_case:
                continue
        new_test_class = comparison_instance['test_class']['text'].replace(comparison_instance['parent_test_case'],
                                                                           new_test_case)
...
if __name__ == '__main__':
    files = [
        'verdict_final-multiple-results-individual-reranked.jsonl',
        'verdict_final-multiple-results-overall-reranked.jsonl',
        'verdict_without_confidence_final-multiple-results-individual-reranked.jsonl',
        "verdict_without_confidence_final-multiple-results-overall-reranked.jsonl",
        ...
        # "first_round_multiple_responses-FourStepCoTGenerator-results.jsonl",
        # "first_round_multiple_responses-NaiveGenerator-results.jsonl",
        # "first_round_multiple_responses-RAGGenerator-results.jsonl",
    ]


    # for v_100 data only
    with open('/Users/yanglin/Documents/Projects/UTGen_LLM/outputs/logprobs_defects4j/verdict_final-multiple-results-individual-reranked.jsonl','r',encoding='utf-8') as reader:
        for line in reader.readlines():
            obj = json.loads(line.strip())
            comparison_group[obj['id']]['test_prefix'] = obj['test_prefix']


    for file in files:
        file = os.path.join('/Users/yanglin/Documents/Projects/UTGen_LLM/outputs/logprobs_defects4j_v100', file)
        data = []
        with open(file, 'r', encoding='utf-8') as reader:
            for line in reader.readlines():
                data.append(json.loads(line.strip()))

        record = cal_pass_rate(data)
        failed_instance_ids = bug_detect(data)
