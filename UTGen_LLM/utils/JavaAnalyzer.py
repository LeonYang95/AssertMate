...
def get_modifier(node: Node) -> str:
    return " ".join(
        [n.text.decode("utf-8") for n in node.children if n.type == "modifiers"]
    )


def _find_package_name(root_node: Node) -> str:
    # 查找 package 定义
    package_decl_node = next(
        filter(lambda n: n.type == "package_declaration", root_node.children), None
    )
    ...
    if package_decl_node:
        try:
            ...
            assert (
                package_decl_node.child(1).type == "scoped_identifier"
            )  # 防止 magic number 出错
...
def _find_class_declaration_node(
    root_node: Node, target_class_name: [str | None]
) -> [None | Node]:
    # 查找目标类定义节点
    ...
    if target_class_name:
        ...
        class_decl_node = next(
            filter(
                lambda n: n.child_by_field_name("name").text.decode("utf-8")
                == target_class_name,
                class_decl_nodes,
            ),
            None,
        ...
        for node in class_decl_nodes:
            modifiers = next(
                filter(lambda n: n.type == "modifiers", node.children), None
            )
            if modifiers:
    ...
    if not class_decl_node:
        logger.warning(
            f"Class declaration not found. Please refer to the debug information for debugging."
        )
...
def method_decl_node_to_method_obj(
    node: Node,
    pkg_name: str,
    class_name: str,
) -> Method:
    ...
    """
    docstring = (
        node.prev_sibling.text.decode("utf-8")
    ...
    parameter_nodes = [
        n
        for n in node.child_by_field_name("parameters").children
        if n.type not in ["(", ")", ","]
    ]
    parameters = []
...
def parse_class_object_from_file_content(
    file_content: str, target_class_name: [str | None] = None
) -> [Class | None]:
    """
    ...
    if superclass_node:
        superclass = class_decl_node.child_by_field_name("superclass").text.decode(
            "utf-8"
        )
    else:
        superclass = ""
    ...
    if super_interface_node:
        interface = class_decl_node.child_by_field_name("interfaces").text.decode(
            "utf-8"
        )
    else:
        interface = ""
...
def extract_method_invocation(
    file_content: str,
    target_class_name: [None | str] = None,
    target_method_name: [None | str] = None,
) -> list:
    tree = parser.parse(bytes(file_content, "utf-8"))
    method_invocations = []
...
def replace_assertion(test_prefix: str, new_assertion: str) -> str:
    ...
    method_decl_node = (
        tree.root_node.children[0].child_by_field_name("body").children[1]
    )
    method_body_node = method_decl_node.child_by_field_name("body")
    query = Language(ts_java.language()).query(
...
def find_params_in_assertion(assertion: str) -> list[str]:
    ...
    try:
        ...
        logger.warning(
            f"Assertion statement {assertion} is not as expected. Expected 3 children, got {len(method_body_node.children)}."
        )
        return []
    ...
    try:
        ...
        logger.warning(
            f"Assertion statement {assertion} is not as expected. Expected method_invocation, got {method_invocation.type}."
        )
        return []
...
def split_test_case_by_assertion(method_str: str) -> list[str]:
    ...
    method_body_node = method_decl_node.child_by_field_name("body")
    query = Language(ts_java.language()).query(
        "(method_invocation name: (_) @name) @invoke"
    )
    ...
    if len(assertion_statement_nodes) == 1:
        ...
        for i in range(len(assertion_statement_nodes)):
            ...
            for j in range(len(assertion_statement_nodes)):
                if j == i:
                    continue
                cur_method = cur_method.replace(
                    assertion_statement_nodes[j].text.decode("utf-8"), ""
                )
            try:
...
def contains_only_one_return_statement(method_str: str):
    tree = parser.parse(bytes("public class ABC {" + method_str + "}", "utf-8"))
    if not tree.root_node.has_error:
        ...
        if (
            method_body_node
            and len(method_body_node.children) == 3
            and method_body_node.child(1).type == "return_statement"
        ):
            return True
        return False
    else:
        logger.error(f"Parsing error in {method_str}")
        return False
